...
from .base_adjustments_cuda import estimate_base_adjustments_cuda
...
def build_merged_lut_probe_style_cuda(
    ...
    """
    Same probe-style ONNX build as pipeline.build_merged_lut_probe_style,
    but color_match.onnx and face ONNX use CUDAExecutionProvider when available.
    """
    source_hwc = prepare_worker_bitmap_source(source_hwc)
    base = estimate_base_adjustments_cuda(source_hwc, worker_feed_prepared=True)
